...
@Component({
    ...
    template: `
        <div #container [ngClass]="{'ui-multiselect ui-widget ui-state-default ui-corner-all':true,'ui-state-focus':focus,'ui-state-disabled': disabled}" [ngStyle]="style" [class]="styleClass"
            ...
            <button style="margin-top:4px" id="dashboardColSelColumns" type="button" class="btn btn-default simple-tooltip autowidth dropdown-toggle" data-title="Customize Columns"><i class="fa fa-columns"></i></button>
            ...
            <div #panel class="ui-multiselect-panel ui-widget ui-widget-content ui-corner-all ui-shadow" [style.display]="overlayVisible ? 'block' : 'none'" (click)="panelClick=true">
                ...
                <div class="ui-multiselect-items-wrapper">
                    <ul class="ui-multiselect-items ui-multiselect-list ui-widget-content ui-widget ui-corner-all ui-helper-reset list-group" dnd-sortable-container [sortableData]="options" [style.max-height]="scrollHeight||'auto'">
                        <li *ngFor="let option of options; let i = index" class="list-group-item ui-multiselect-item ui-corner-all" (click)="onItemClick($event,option.value)" 
                            [style.display]="isItemVisible(option) ? 'block' : 'none'" [ngClass]="{'ui-state-highlight':isSelected(option.value)}"  (onDragSuccess)="getSortedData($event)" dnd-sortable  [sortableIndex]="i">
                            <div class="ui-chkbox ui-widget">
                                <div class="ui-helper-hidden-accessible">
...
export class MultiSelect implements OnInit,AfterViewInit,AfterViewChecked,DoCheck,OnDestroy,ControlValueAccessor {
    ...
    @Output() sortedDataChange : EventEmitter<any> = new EventEmitter();
    ...
    onItemClick(event, value) {
        ...
        this.value = _.filter(this.value,function (value) {
    ...
    getSortedData(event: any = []){ 
        ...
        if(that.value && that.value.length > 0){
            sortedData = _.filter(this.options, function(val){
                return _.find(that.value, val.value);
            });
        } else {
            sortedData = this.options;
        }        
...
export class GhxMultiSelectModule { }
